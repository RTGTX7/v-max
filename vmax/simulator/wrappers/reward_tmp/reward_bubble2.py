...
class RewardCustomWrapper(Wrapper):
    ...
    def _to_score(name: str, v) -> jax.Array:
        ...
        # ---- 连续质量 ----
        if name == "safe_bubble":
            # v 已经是 score ∈ [min_score, 1]，直接返回即可
            return jnp.asarray(v, dtype=jnp.float32)

        if name == "route_safe":
            # 组合后的 route_safe 已经是 [min_score, 1] 分数
            return jnp.asarray(v, dtype=jnp.float32)
...
def _get_reward_fn(reward_name: str):
    table = {
        ...
        "forward_quality": _compute_forward_quality_reward,
        "comfort": _compute_comfort_reward,
        "safe_bubble": _compute_safe_bubble_reward,
        "route_safe": _compute_route_safe_reward,
        "termination_switch": _compute_termination_switch_reward,
    }
...
def _compute_off_route_reward(state: datatypes.SimulatorState, **_) -> jax.Array:
    ...
    return jnp.asarray(jnp.maximum(off_route_score, 0.0), dtype=jnp.float32)


def _compute_termination_switch_reward(state: datatypes.SimulatorState, **_) -> jax.Array:
    offroad = _compute_offroad_reward(state)
    overlap = _compute_overlap_reward(state)
    red_light = _compute_red_light_reward(state)
    violated = jnp.logical_or(offroad > 0.0, jnp.logical_or(overlap > 0.0, red_light > 0.0))
    return jnp.asarray(violated, dtype=jnp.float32)
...
def _compute_safe_bubble_reward(
    ...
    **_,
    ...
    # 关键：lazy import，避免 circular import
    from vmax.simulator.metrics.safe_bubble import soft_safe_bubble_score

    score = soft_safe_bubble_score(
        state,
        ttc_front=ttc_front,
        ttc_rear=ttc_rear,
        d_static=d_static,
    ...
    return jnp.clip(jnp.asarray(score, dtype=jnp.float32), min_score, 1.0)


def _compute_route_safe_reward(
    state: datatypes.SimulatorState,
    # ---- bubble shape knobs ----
    ttc_front: float = 3.0,
    ttc_rear: float = 1.5,
    ...
    penetration_max_m: float = 8.0,
    # ---- route shaping ----
    off_route_beta: float = 1.0,
    ignore_overlap_in_safe_bubble: bool = True,
    **_,
) -> jax.Array:
    safe_score = _compute_safe_bubble_reward(
        state,
        ttc_front=ttc_front,
        ttc_rear=ttc_rear,
        d_static=d_static,
        ...
        side_margin_max=side_margin_max,
        anisotropy=anisotropy,
        p_base=p_base,
        delta_p=delta_p,
        time_horizon_s=time_horizon_s,
    ...
    off_route_raw = _compute_off_route_reward(state)
    off_route_soft = 1.0 / (1.0 + jnp.maximum(off_route_raw, 0.0) * off_route_beta)
    route_safe_score = jnp.asarray(safe_score * off_route_soft, dtype=jnp.float32)
    if ignore_overlap_in_safe_bubble:
        overlap = _compute_overlap_reward(state)
        route_safe_score = jnp.where(overlap > 0.0, jnp.asarray(1.0, dtype=jnp.float32), route_safe_score)
    return jnp.clip(route_safe_score, min_score, 1.0)
...
def _compute_forward_quality_reward(state: datatypes.SimulatorState, **_) -> jax.Array:
    progression = _compute_making_progress_reward(state)
    comfort = _compute_comfort_reward(state)
    return jnp.asarray(jnp.clip(progression * comfort, 0.0, 1.0), dtype=jnp.float32)
